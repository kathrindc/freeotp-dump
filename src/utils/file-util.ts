...
const getPassKey = async (passphrase: string, saltArray: number[], iterations: number, hashAlgo: AlgorithmIdentifier) => {
    ...
    const passCrypt = await crypto.subtle.importKey("raw", passBuffer, "PBKDF2", false, ["deriveBits"]);
    const passKeyBytes = await crypto.subtle.deriveBits(pbkdf2, passCrypt, saltArray.length * 8);
    const aesGcm: AesGcmParams = {
        name: "AES-GCM",
        iv: new ArrayBuffer(12),
    };
    
    return await crypto.subtle.importKey("raw", passKeyBytes, aesGcm, false, ["decrypt"]);
};
...
const getMasterKey = async (master: MFAMasterKey, passphrase: string) => {
    ...
    const passKey = await getPassKey(passphrase, master.mSalt, master.mIterations, "SHA-512");
    ...
    const masterKeyBytes = await crypto.subtle.decrypt(masterAesGcm, passKey, encryptedData);
    const importAesGcm: AesGcmParams = {
        name: "AES-GCM",
        iv: new ArrayBuffer(12),
    };

    return await crypto.subtle.importKey("raw", masterKeyBytes, importAesGcm, false, ["decrypt"]);
};
...
export const decrypt = async (raw: MFARawCollection, passphrase: string) => {
    ...
    for (let rawToken of raw.tokens) {
        ...
        const keyData = await crypto.subtle.decrypt(aesGcm, masterKey, encryptedBuffer);
