...
export class JavaReader {
    ...
    collect() {
        ...
        for (let i = 0; i < this.count; ++i) {
            ...
            if (keyType != 0x74) {
                throw new JavaError(this.buffer, this.position, `Unsupported key type 0x${keyType.toString(16)}`);
            }

            let key = this.utf();

            if (this.result[key]) {
                throw new JavaError(this.buffer, this.position, `Key "${key}" exists more than once`);
            }

            let valueType = this.u8();

            if (valueType != 0x74) {
                throw new JavaError(this.buffer, this.position, `Unsupported value type 0x${valueType.toString(16)}`);
            }
